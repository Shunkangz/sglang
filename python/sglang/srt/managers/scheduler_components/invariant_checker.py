...
class SchedulerInvariantChecker:
    ...
    def _check_full_pool(self, ps: PoolStats, uncached: int = 0) -> Tuple[bool, str]:
        ...
        if widened_page_alloc:
            ...
            # Both DCP and sharding store logical tokens in widened allocator
            # pages. Prefix cache counters are logical-token based, while the
            # allocator frees whole pages, so round cached tokens up to
            # allocator page units.
